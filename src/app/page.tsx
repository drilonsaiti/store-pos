...
import { Package, Boxes, Wallet, ShoppingBag, TrendingUp, AlertTriangle, Plus, ScanBarcode, Receipt } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      lowStock: p.filter((x) => getStockStatus(x.quantity) !== 'in-stock'),
      ...
      <div className="flex flex-col gap-6 p-4 md:p-6">
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
            ...
            <KpiCard label="Today's revenue" value={formatCurrency(metrics.todayRevenue)} icon={TrendingUp} />
            <KpiCard label="Low stock" value={String(metrics.lowStock.length)} icon={AlertTriangle} tone="warning" />
          </div>
        )}
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-2 pt-0">
              {metrics.lowStock.length === 0 && !isLoading && (
                <p className="text-sm text-muted-foreground">Everything is above the {LOW_STOCK_THRESHOLD}-unit threshold.</p>
              )}
