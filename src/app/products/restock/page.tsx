...
import { useProducts, useBarcodeIndex, useIncrementProductQuantity } from '@/hooks/use-products';
...
export default function RestockPage() {
    ...
    const handleConfirmRestock = async (amountToAdd: number) => {
        ...
        const newTotal = await incrementQuantity.mutateAsync({ id: pendingProduct.id, delta: amountToAdd });
        playScanSuccess();
        setLog((prev) => [
            {
                ...
                added: amountToAdd,
                unit: pendingProduct.saleUnit === 'weight' ? pendingProduct.weightUnit ?? 'kg' : 'pcs',
                newTotal,
    ...
    const handleUndo = async (entry: LogEntry) => {
        await incrementQuantity.mutateAsync({ id: entry.productId, delta: -entry.added });
        setLog((prev) => prev.map((e) => (e.key === entry.key ? { ...e, undone: true } : e)));
    };
        ...
        <AppShell title="Add inventory">
            <div className="p-4 md:p-6">
                <p className="mb-4 text-sm text-muted-foreground">
                    Scan or search a product, enter how many (or how much) arrived, and it's added on top of the current
                    stock — nothing here overwrites existing quantity.
                </p>
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row">
                      ...
                      <span>
                        {p.name} <span className="tabular text-muted-foreground">· {p.barCode}</span>
                                            ...
                                            <span className="tabular text-muted-foreground">{p.quantity} in stock</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                ...
                {notFoundCode && (
                    <Card className="mb-4 border-warning/40 bg-warning/5">
                        <CardContent className="flex flex-col items-center gap-3 py-6 text-center">
                            ...
                            <p className="tabular text-sm text-muted-foreground">{notFoundCode}</p>
                            <Button asChild size="sm">
                                <Link href={`/products/new?barcode=${encodeURIComponent(notFoundCode)}`}>Create product</Link>
                            </Button>
                        </CardContent>
                    </Card>
                )}

                <h2 className="mb-2 text-sm font-medium text-muted-foreground">This session</h2>
                {log.length === 0 ? (
                    <EmptyState icon={PackageCheck} title="Nothing added yet" description="Items you restock will show up here." />
                ) : (
                    <div className="flex flex-col gap-2">
                        {log.map((entry) => (
                            <Card key={entry.key} className={`p-3 ${entry.undone ? 'opacity-50' : ''}`}>
                                <div className="flex items-center justify-between gap-3">
                                    <div className="min-w-0 flex-1">
                                        <p className="truncate text-sm font-medium">{entry.name}</p>
                                        <p className="tabular text-xs text-muted-foreground">
                                            {formatDateTime(entry.at)} · +{entry.added} {entry.unit} · now {entry.newTotal} {entry.unit}
                                            {entry.undone && ' · undone'}
                                        </p>
                                    </div>
                                    {!entry.undone && (
                                        <Button variant="ghost" size="icon" aria-label={`Undo restock of ${entry.name}`} onClick={() => handleUndo(entry)}>
