...
export function useCameraBarcodeScanner({ onDetect, debounceMs = 500, enabled, engineId = 'auto' }: Options) {
    ...
    const [activeEngineId, setActiveEngineId] = useState<Exclude<ScannerEngineId, 'auto'> | null>(null);
    ...
    useEffect(() => {
        ...
        async function start() {
            ...
            try {
                ...
                if (engine.mode === 'continuous' && engine.startContinuous && videoRef.current) {
                    continuousControlsRef.current = await engine.startContinuous(videoRef.current, acceptDetection);
                    return;
                }
                ...
                intervalRef.current = setInterval(async () => {
                    ...
                    if (!video || video.readyState < 2 || !activeEngine?.detectFrame) return;
                    try {
                        const useFullFrame = useFullFrameNextRef.current;
                        useFullFrameNextRef.current = !useFullFrameNextRef.current;

                        if (useFullFrame) {
                            const code = await activeEngine.detectFrame(video);
                            if (code) acceptDetection(code);
                            return;
                        }
